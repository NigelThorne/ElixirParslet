defmodule ParsletV1 do
    ...
    quote do
      def parse(document) do
        ...
        case apply(__MODULE__, @root, []).(document) do
          ...
          {:ok, any, rest} ->
            {:error, "Consumed #{inspect(any)}, but had the following remaining '#{rest}'"}
  ...
  defp str_aux(text, doc, matched) do
    ...
    if String.starts_with?(doc, text) do
      {:ok, matched <> text, String.slice(doc, tlen..-1//1)}
    else
  ...
  defp match_aux(regex_s, doc, matched) do
    ...
    case Regex.run(regex, doc) do
      ...
      [match | _] -> {:ok, matched <> match, String.slice(doc, String.length(match)..-1//1)}
    end
  end
